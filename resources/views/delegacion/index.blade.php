...
@section('content')
<a href="delegaciones/create" class="btn btn-primary mb-3">Añadir Delegación</a>

<table id="delegaciones" class="table table-striped table-bordered shadow-lg mt-4" style="width:100%">
    <thead class="bg-primary text-white">
        <tr>
            <th scope="col">Nº Delegación</th>
    ...
    <tbody>
        ...
        <tr>
            <td>{{ $delegacion-> id }}</td>
            <td>{{ $delegacion-> nombreEmpresa }}</td>
            <td>{{ $delegacion-> localidad }}</td>
            <td>
                <form action="{{ route ('delegaciones.destroy',$delegacion->id) }}" method="POST">
                    <a href="/delegaciones/{{ $delegacion-> id }}/edit" class="btn btn-info">Editar</a>
                    @csrf
                    @method('DELETE')
